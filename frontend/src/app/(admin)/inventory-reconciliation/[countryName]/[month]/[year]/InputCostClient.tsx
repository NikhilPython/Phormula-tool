...
//             onAnyGroupExpandedChange={setAnyExpanded}
...
export default function InventoryReconciliationPage({ params }: Params) {
  ...
  const getVisibleExportCols = useCallback(() => {
    ...
    for (const c of leftCols) cols.push({ key: c.key, header: c.label });
    ...
    for (const g of groups) {
      ...
      for (const c of leafCols) {
        cols.push({
          key: c.key,
          header: `${g.label} - ${c.label}`,
        });
      }
    }

    for (const c of singleCols) cols.push({ key: c.key, header: c.label });

...
#     # ============================================================
#     # ✅ SKU-WISE TABLE (MONTH NAME) + ADS MERGE + NEW COLUMNS:
#     #    - product_spend, display_spend, brand_spend
#     #    - brand_spend is added to GRAND_TOTAL from adsmonthly Grand Total row
#     # ============================================================

#     # ✅ store like: skuwisemonthly_1_uk_february_2026
...
#             # ✅ pull totals from adsmonthly "Grand Total" row (best)
...
#         # ensure standard ads columns exist + numeric
#         for col in [
#             "ads_impressions",
#             "ads_clicks",
#             "ads_spend",
...
#         # ✅ ensure NEW spend split columns exist + numeric
...
#         if "net_sales" not in df_sku.columns:
#             df_sku["net_sales"] = 0.0
...
#             lambda r: (float(r["cm2_profit"]) / float(r["quantity"])) if float(r.get("quantity") or 0) else 0.0,
#             axis=1,
#         )
#         df_sku["cm2_profit_per"] = df_sku.apply(
#             lambda r: (float(r["cm2_profit"]) / float(r["net_sales"]) * 100.0) if float(r.get("net_sales") or 0) else 0.0,
#             axis=1,
#         )
...
#                 elif "product_name" not in df_sku.columns and "product_name_x" in df_sku.columns:
#                     df_sku.rename(columns={"product_name_x": "product_name"}, inplace=True)
...
#         # -------- GRAND TOTAL --------
#         total_row: Dict[str, Any] = {"sku": "GRAND_TOTAL", "product_name": "Grand Total"}
...
#         # ✅ IMPORTANT: use adsmonthly totals so brand_spend is not lost
...
#         g_qty = float(total_row.get("quantity", 0.0))
...
#             ads_agg["ads_conversion_rate"] = ads_agg.apply(
#                 lambda r: (float(r["sale_units"]) / float(r["clicks"]) * 100.0) if float(r["clicks"]) else 0.0, axis=1
#             )
#             ads_agg["ads_roas"] = ads_agg.apply(
#                 lambda r: (float(r["sale_amount"]) / float(r["spend"])) if float(r["spend"]) else 0.0, axis=1
#             )
#             ads_agg["ads_acos"] = ads_agg.apply(
#                 lambda r: (float(r["spend"]) / float(r["sale_amount"]) * 100.0) if float(r["sale_amount"]) else 0.0, axis=1
#             )
...
#         for col in ["profit","quantity","net_sales"]:
#             if col not in df_sku.columns:
#                 df_sku[col] = 0.0
#             df_sku[col] = pd.to_numeric(df_sku[col], errors="coerce").fillna(0.0)
...
#         df_sku["cm1_profit_per_unit"] = df_sku.apply(
#             lambda r: (float(r["profit"]) / float(r["quantity"])) if float(r["quantity"]) else 0.0, axis=1
#         )
#         df_sku["cm1_profit_per"] = df_sku.apply(
#             lambda r: (float(r["profit"]) / float(r["net_sales"]) * 100.0) if float(r["net_sales"]) else 0.0, axis=1
#         )
#         df_sku["cm2_profit_per_unit"] = df_sku.apply(
#             lambda r: (float(r["cm2_profit"]) / float(r["quantity"])) if float(r["quantity"]) else 0.0, axis=1
#         )
#         df_sku["cm2_profit_per"] = df_sku.apply(
#             lambda r: (float(r["cm2_profit"]) / float(r["net_sales"]) * 100.0) if float(r["net_sales"]) else 0.0, axis=1
#         )
...
#                 map_df["sku_key"] = map_df["sku_key"].fillna("").astype(str).str.strip()
#                 map_df["product_name"] = map_df["product_name"].fillna("").astype(str).str.strip()
#                 map_df = map_df[map_df["sku_key"] != ""].sort_values(by=["sku_key"]).drop_duplicates(subset=["sku_key"], keep="first")
...
def finances_mtd_transactions():
    ...
    if not df_all.empty:
        ...
        for col in ["product_spend", "display_spend", "brand_spend"]:
            df_sku[col] = pd.to_numeric(df_sku.get(col, 0.0), errors="coerce").fillna(0.0)
        ...
        # cm2 / metrics
        df_sku["profit"] = pd.to_numeric(df_sku.get("profit", 0.0), errors="coerce").fillna(0.0)
        df_sku["cm2_profit"] = (df_sku["profit"] - df_sku["ads_spend"]).fillna(0.0)

...
//         // initialize all template fields so table order stays fixed
...
//     return TEMPLATE_COLUMNS.map((key) => ({
//       key,
...
//         const validationError = validateTemplateColumns(cleaned);
...
//   const validateTemplateColumns = (parsedRows: Row[]) => {
//     if (!parsedRows.length) return "The uploaded file is empty.";

//     const firstRowKeys = Object.keys(parsedRows[0]);
//     const missing = TEMPLATE_COLUMNS.filter((col) => !firstRowKeys.includes(col));

//     if (missing.length) {
//       return `Missing required columns: ${missing
//         .map((col) => HEADER_LABELS[col])
//         .join(", ")}`;
//     }
...
//     a.href = `/SKU%20Information.xlsx`;
//     a.download = "SKU Information.xlsx";

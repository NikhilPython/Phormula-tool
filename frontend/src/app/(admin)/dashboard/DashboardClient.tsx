...
// // //       cm1_profit_per: Number(r.cm1_profit_per ?? 0),   // ✅ NEW
// // //       cm1_profit_per_unit: Number(r.cm1_profit_per_unit ?? 0),   // ✅ NEW
...
// // //               // { type: "single", key: "cm1_profit_per_unit" },
// // //               // { type: "single", key: "cm1_profit_per" },
...
// // //               if (colKey === "product_name") return row.isTotal ? "Grand Total" : row.product_name;
...
// // //                 return formatAdsNumber(row.cm1_profit_per);

// // //               if (colKey === "cm1_profit_per_unit")
// // //                 return formatAdsNumber(row.cm1_profit_per_unit);
...
// //   return { start_date: toISO(start), end_date: toISO(yesterday) };
// // };
...
// //   // 202 = pending (backend starts async job) — treat as "seed started"
// //   if (res.status == 202) {
// //     localStorage.setItem(storageKey, "1");
// //     return;
// //   }
...
// //   // Seed SP + SD advertised product reports once (per user/month) when Ads is connected
// //   const adsSeedOnceRef = useRef(false);

// //   useEffect(() => {
// //     if (adsSeedOnceRef.current) return;
// //     adsSeedOnceRef.current = true;
...
// //         if (statusJson?.status !== "connected") return;

// //         const now = new Date();
// //         const monthStr = monthNamesLower[now.getMonth()] || "january";
// //         const yearStr = String(now.getFullYear());

// //         // NOTE: keeping country="UK" as per the previous AppSidebar logic
// //         await ensureSpReportSeedOnce(baseURL, jwtToken, monthStr, yearStr, "UK");
// //         await ensureSdReportSeedOnce(baseURL, jwtToken);
// //       } catch (err) {
// //         console.error("Ads seed error:", err);
// //       }
// //     };
...
// // Ads seeding endpoints
// const ADS_SP_SEED_ENDPOINT = `${baseURL}/api/ads/manager/sp_advertised_product_report`;
// const ADS_SD_SEED_ENDPOINT = `${baseURL}/api/ads/manager/sd_advertised_product_report/sync`;
...
// // ===================== ADS: DAILY SEEDING + MONTHLY DB SYNC =====================
// // Daily key is computed in IST (Asia/Kolkata) so "once per day" matches your business day.
// const getISTDateKey = () => {
//   const now = new Date();
//   const ist = new Date(now.toLocaleString("en-US", { timeZone: "Asia/Kolkata" }));
...
// const getAuthHeaders = (jwtToken: string): HeadersInit => ({
//   "Content-Type": "application/json",
//   Accept: "application/json",
//   Authorization: `Bearer ${jwtToken}`,
// });

// const safePostJson = async (url: string, jwtToken: string, body?: any) => {
//   const res = await fetch(url, {
//     method: "POST",
...
// const safeGetJson = async (url: string, jwtToken: string) => {
//   const res = await fetch(url, {
//     method: "GET",
//     headers: {
//       Authorization: `Bearer ${jwtToken}`,
...
// const getAdsCountryFromPlatform = (platform: PlatformId) => {
//   if (platform === "amazon-us") return "US";
//   if (platform === "amazon-ca") return "CA";
//   // Keep UK as default (matches prior AppSidebar behavior, and also used for global).
//   return "UK";
// };

// const getUserIdForAdsKeys = (userData: any) => {
//   // best-effort: supports multiple shapes
//   return (
//     userData?.id ??
//     userData?.user_id ??
//     userData?._id ??
...
//   // ===================== ADS: DAILY SEED → THEN MONTHLY DB SYNC =====================
//   // First time each day (IST):
//   //   1) sp_advertised_product_report
//   //   2) sd_advertised_product_report/sync
//   //   3) monthly_sp_sd_to_db
//   // After that (same day):
//   //   only monthly_sp_sd_to_db
//   useEffect(() => {
//     let cancelled = false;

//     const runAdsFlow = async () => {
//       const jwtToken = typeof window !== "undefined" ? localStorage.getItem("jwtToken") : null;
//       if (!jwtToken) return;

//       // Only run if Ads is connected
//       let statusJson: any;
//       try {
//         statusJson = await safeGetJson(`${baseURL}/api/ads/status`, jwtToken);
//       } catch {
//         return;
//       }
//       if (cancelled) return;
...
//       const { monthName, year } = getISTYearMonth();
//       const month = monthToNumber(String(monthName || "january").toLowerCase());
//       const adsCountry = getAdsCountryFromPlatform(platform as PlatformId);
//       const userId = getUserIdForAdsKeys(userData);
//       const dayKey = getISTDateKey();
//       const seedKey = `ads_seed_daily_${userId}_${adsCountry}_${year}_${month}_${dayKey}`;

//       try {
//         // First time today → seed SP then SD, then run monthly.
//         if (localStorage.getItem(seedKey) !== "1") {
//           await safePostJson(ADS_SP_SEED_ENDPOINT, jwtToken, { month, year, country: adsCountry });
//           await safePostJson(ADS_SD_SEED_ENDPOINT, jwtToken, { month, year, country: adsCountry });
//           localStorage.setItem(seedKey, "1");
//         }

//         // Always run monthly after the above (seeded or already seeded today)
//         await safePostJson(MONTHLY_SP_ENDPOINT, jwtToken, { month, year, country: adsCountry });
//       } catch (err) {
//         // Don't block dashboard if Ads jobs fail.
//         console.error("Ads daily flow error:", err);
//       }
//     };

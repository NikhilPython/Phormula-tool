import type { Metadata } from "next";
import IntegrationDashboard from "@/features/integration/IntegrationDashboard";

type Params = {
  countryName: string;
  month?: string;
  year?: string;
};

// helper (optional, same as your previous file)
const monthName = (m: string) => {
  const v = String(m).toLowerCase();
  const map: Record<string, string> = {
    "01": "January", "1": "January", "jan": "January",
    "02": "February", "2": "February", "feb": "February",
    "03": "March", "3": "March", "mar": "March",
    "04": "April", "4": "April", "apr": "April",
    "05": "May", "5": "May",
    "06": "June", "6": "June", "jun": "June",
    "07": "July", "7": "July", "jul": "July",
    "08": "August", "8": "August", "aug": "August",
    "09": "September", "9": "September", "sep": "September",
    "10": "October", "oct": "October",
    "11": "November", "nov": "November",
    "12": "December", "dec": "December",
  };
  return map[v] ?? m;
};

const formatCountry = (c: string) => {
  const v = (c || "").toLowerCase();
  if (v === "uk") return "UK";
  if (v === "us") return "US";
  if (v === "india") return "India";
  if (v === "ca") return "Canada";
  if (v === "global") return "Global";
  return v.toUpperCase();
};

// ✅ Metadata
export async function generateMetadata(
  { params }: { params: Promise<Params> }
): Promise<Metadata> {

  const { countryName, month = "", year = "" } = await params;

  const country = formatCountry(countryName);
  const monthFormatted =
    month && month.toLowerCase() !== "na"
      ? monthName(decodeURIComponent(month))
      : "";

  const showDate =
    month?.toLowerCase() !== "na" &&
    year?.toLowerCase() !== "na";

  return {
    title: `Integration Dashboard | Amazon ${country}`,
    description: showDate
      ? `Manage integrations for ${country} in ${monthFormatted} ${year}.`
      : `Manage integrations for ${country}.`,
    robots: { index: false, follow: false },
  };
}

// ✅ Page
export default async function IntegrationDashboardPage(
  { params }: { params: Promise<Params> }
) {
  const { countryName, month, year } = await params;

  // ✅ If month & year are "NA" → simple dashboard
  if (
    month?.toLowerCase() === "na" &&
    year?.toLowerCase() === "na"
  ) {
    return <IntegrationDashboard />;
  }

  // ✅ Otherwise pass values
  return (
    <IntegrationDashboard
      
    />
  );
}
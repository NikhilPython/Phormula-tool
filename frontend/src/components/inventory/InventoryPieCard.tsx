...
export default function InventoryPieCard({
  ...
  const displayData = useMemo(() => {
    // ✅ remove zero values to avoid clutter
    return (data || []).map(d => ({ ...d, value: Math.abs(toNum(d.value)) })).filter(d => d.value > 0);
  }, [data]);
  ...
  const options = useMemo<ChartOptions<"pie">>(() => {
    return {
      ...
      radius: isLaptop ? "96%" : isDesktop ? "90%" : "96%",
      // ✅ IMPORTANT: remove external padding so no overlap
      layout: { padding: 0 },
      plugins: {
        // ✅ default legend OFF (we render our own)
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-4 flex flex-col">
      <div className="text-base font-semibold text-[#414042]">{title}</div>
        ...
        <div className="mt-3 flex-1 min-h-0 w-full">
          <div className="relative w-full flex flex-col xl:flex-row gap-4 xl:gap-6 items-stretch xl:items-center">
            ...
            <div
              ...
              style={{
                width: isDesktop ? 280 : isLaptop ? 240 : "100%",
                maxHeight: height,
              }}
            >

...
export default function SignInForm() {
  ...
  const routeToDashboard = (
    country: string,
    amazonUserExists: boolean = true
  ) => {
    const now = new Date();
    const currentMonth = amazonUserExists
      ? now.toLocaleString("en-US", { month: "long" })
      : "NA";
    const currentYear = amazonUserExists
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (loginType === "client") {
        ...
        const amazonUserExists = me?.amazon_user_exits === true;

        routeToDashboard(countryFromBackend, amazonUserExists);
        return;
      }

...
// //   // ✅ build arrays aligned to allDays
// //   const buildSeries = (data: DailyPoint[]) =>
// //     allDays.map((day) => {
// //       const pt = data.find((d) => getDay(d.date) === day);
// //       return pt ? (metric === "quantity" ? pt.quantity ?? null : pt.net_sales ?? null) : null;
...
// //   const prevData = useMemo(() => buildSeries(dataPrev), [dataPrev, allDays, metric]);
// //   const currData = useMemo(() => buildSeries(dataCurr), [dataCurr, allDays, metric]);

// //   // ✅ SINGLE DAY FIX: If only one day on x-axis, add a hidden pad day & duplicate values (so a line renders)
// //   const PAD_X = "__single_point_pad__";
// //   const isPadX = (x: any) => String(x) === PAD_X;

// //   const effectiveXAxis = useMemo(() => {
// //     const base = allDays.map(String);
// //     if (base.length === 1) return [base[0], PAD_X];
// //     return base;
// //   }, [allDays]);

// //   const effectivePrev = useMemo(() => {
// //     if (allDays.length !== 1) return prevData;
// //     return [prevData[0] ?? null, prevData[0] ?? null];
// //   }, [allDays.length, prevData]);

// //   const effectiveCurr = useMemo(() => {
// //     if (allDays.length !== 1) return currData;
// //     return [currData[0] ?? null, currData[0] ?? null];
// //   }, [allDays.length, currData]);
...
// //   // responsive sizing (same as your code)
...
// //   // ✅ marker styling (filled circles + bigger on hover)
// //   const SYMBOL_SIZE = 7;
...
// //         if (isPadX(day)) day = effectiveXAxis[0]; // ✅ treat pad as the real day

// //         const stripRange = (name: string) => (name || "").replace(/\s*\d+\s*[–-]\s*\d+\s*$/g, "");

// //         const lines = (params || [])
// //           .filter((p: any) => !isPadX(p?.axisValue)) // ✅ hide pad in tooltip
// //           .map((p: any) => {
// //             const val = p.data;
// //             const seriesName = stripRange(p.seriesName);
...
// //         formatter: (v: string) => (isPadX(v) ? "" : String(v)), // ✅ hide pad tick
// //       },
// //       nameTextStyle: { fontSize: axisNameFontSize },
// //     },
...
// //         itemStyle: { borderWidth: 0 }, // filled (uses series color)
...
// //   // resize handling like your other chart
// //   useEffect(() => {
...
//   // ✅ Single-point pad (same as your PerformanceTrendChart pattern)
//   const PAD_X = "__single_point_pad__";
//   const isPadX = (x: unknown) => String(x) === PAD_X;
...
//   // Build series data AGAINST effectiveXAxis (important)
//   const effectivePrev = useMemo(() => {
//     if (allDays.length !== 1) return prevData;
//     const v0 = prevData[0] ?? null;
//     return [v0, v0];
//   }, [allDays.length, prevData]);

//   const effectiveCurr = useMemo(() => {
//     if (allDays.length !== 1) return currData;
//     const v0 = currData[0] ?? null;
//     return [v0, v0];
//   }, [allDays.length, currData]);
...
//           let day: string | number = params?.[0]?.axisValue ?? "";
//           if (isPadX(day)) day = effectiveXAxis[0]; // treat pad as real day

//           const stripRange = (name: string) => (name || "").replace(/\s*\d+\s*[–-]\s*\d+\s*$/g, "");

//           const lines = params
//             .filter((p) => !isPadX(p?.axisValue)) // hide pad row
//             .map((p) => {
//               const val = p.data;
//               const seriesName = stripRange(p.seriesName ?? "");
//               const shownNum = typeof val === "number" ? val : null;
...
//           // ✅ data length matches effectiveXAxis; pad duplicates first value
//           data: effectivePrev.map((v, idx) => (isPadX(effectiveXAxis[idx]) ? effectivePrev[0] : v)),
//         },
//         {
//           name: currLabel || "Current MTD",
//           type: "line",
...
//           data: effectiveCurr.map((v, idx) => (isPadX(effectiveXAxis[idx]) ? effectiveCurr[0] : v)),
//         },
//       ],
//     }),
//     [

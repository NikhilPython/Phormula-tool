...
export default function ChatbotCore({
  ...
  // ✅ NEW: direct reference to textarea
  ...
  function convertPlainTextToMarkdown(
    text: string
  ): string {
    ...
    for (const raw of lines) {
      ...
      if (
        /^CONSOLIDATED ACTIONS$/i.test(line)
      ) {
        flushProductPoints();
    ...
    if (
      consolidatedPoints.length > 0
    ) {
      consolidatedPoints.forEach((p) =>
        out.push(`1. ${p}`)
      );
    }
  ...
  const handleSendMessage = () => {
    ...
    // Empty message or already loading
    if (!value || loading) {
      return;
    }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <p className="text-[10px] text-center text-gray-400 pb-2 shrink-0">
        AI-generated responses. Verify critical
        info.
      </p>
    </div>
  );

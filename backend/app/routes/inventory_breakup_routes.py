...
def inventory_lost_compensation():
    ...
    country = (request.args.get("country") or "").strip().lower()
    year = (request.args.get("year") or "").strip()
    mode = (request.args.get("mode") or "month").strip().lower()   # month | year
    month = (request.args.get("month") or "").strip().lower()
    ...
    if mode not in {"month", "year"}:
        return jsonify({"success": False, "error": "mode must be month or year"}), 400
    ...
    try:
        ...
        # -------------------------------------------------
        # AMAZON DB: inventory monthly or yearly
        # -------------------------------------------------
        with amazon_engine.connect() as conn:
            exists_sql = text("""
                SELECT EXISTS (
    ...
    # -------------------------------------------------
    # Build final response + rows to store
    # -------------------------------------------------
    data = []
    rows_to_store = []
    summary = {
    ...
    # -------------------------------------------------
    # Store result table in amazon db
    # -------------------------------------------------
    try:
        with amazon_engine.begin() as conn:
            # optional: drop old table if you want fresh schema
            # conn.execute(text(f"DROP TABLE IF EXISTS {result_table}"))
            ...
            insert_sql = text(f"""
                INSERT INTO {result_table} (
                    sort_order,
                    user_id, country, year, mode, month,

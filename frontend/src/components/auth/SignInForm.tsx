...
export default function SignInForm() {
  ...
  const fetchAmazonConnections = async (
    token: string
  ): Promise<AmazonConnection[]> => {
    try {
      const res = await fetch(`${API_BASE}/amazon_api/connections`, {
        method: "GET",
        headers: {
      ...
      console.error("fetchAmazonConnections error:", error);
      return [];
    }
  };

  const getConnectedAmazonConnections = (
    connections: AmazonConnection[] = []
  ) => {
    return connections.filter(
      (connection) => connection?.is_connected === true
    );
  };

  const getPrimaryCountryFromConnections = (
    connections: AmazonConnection[] = []
  ) => {
    const connectedConnections = getConnectedAmazonConnections(connections);

    if (connectedConnections.length > 0 && connectedConnections[0]?.country) {
      return String(connectedConnections[0].country).trim().toLowerCase();
    }
  ...
  const routeMemberByModule = (country: string, modules: string[] = []) => {
    ...
    for (const mod of modules) {
      if (mod === "LIVE_DASHBOARD") {
        router.replace(
          `/live-dashboard/${normalizedCountry}/${month}/${year}`
        );
        return;
      }
      ...
      if (mod === "FINANCE_DASHBOARDS") {
        router.replace(
          `/pnl-dashboard/MTD/${normalizedCountry}/${month}/${year}`
        );
        return;
      }
      ...
      if (mod === "BUSINESS_INTELLIGENCE") {
        router.replace(
          `/ai-insight/MTD/${normalizedCountry}/${month}/${year}`
        );
        return;
      }
      ...
      if (mod === "INVENTORY_PLANNING") {
        router.replace(
          `/inventory-reconciliation/${normalizedCountry}/${month}/${year}`
        );
        return;
      }
    }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!result?.token) {
        dispatch(setAuthError("Login failed. No token returned from server."));
        return;
      }
      ...
      if (loginType === "client") {
        ...
        dispatch(
          setUser({
            ...
            email: cleanEmail,
            is_member: false,
            amazon_connections: connections,
            countries: connectedConnections
              .map((connection) => connection.country)
              .filter(Boolean),
            marketplace_ids: connectedConnections
              .map((connection) => connection.marketplace_id)
              .filter(Boolean),
          })
        );

        if (!hasAmazonConnection) {
          routeToNoDataDashboard(countryFromAmazon);
          return;
        }

        if (!userId) {
          console.error(
            "User ID missing from login response. Cannot check user country table."
          );
          routeToNoDataDashboard(countryFromAmazon);
          return;
        }
        ...
        routeToDashboard(countryFromAmazon);
        return;
      }
      ...
      const memberModules = Array.isArray(result?.modules)
        ? result.modules
        : [];
  ...
  const onGoogleSignIn = async () => {
    ...
    try {
      ...
      if (!userId) {
        console.error(
          "User ID missing from Google login response. Cannot check user country table."
        );
        routeToNoDataDashboard(countryFromAmazon);
        return;
      }

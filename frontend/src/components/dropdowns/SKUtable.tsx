...
//       // Table column "Other Transactions" should show other_transaction_fees at row level
//       other_transactions: toNumber(row.other_transaction_fees),
...
//   // ✅ dynamic visible column count coming from GroupedCollapsibleTable
//   const [mainColCount, setMainColCount] = useState(0);
...
//   const MAIN_SINGLE_COLS: LeafCol<TableRow>[] = useMemo(
//     () => [
//       { key: "asp", label: "ASP", align: "center" },
//       { key: "net_sales", label: "Net Sales", align: "center" },
//       { key: "cost_of_unit_sold", label: "COGS", align: "center" },
...
//   // ✅ Total columns in the main (collapsed) table grid
// const SUMMARY_COL_COUNT = useMemo(() => LEFT_COLS.length + MAIN_SINGLE_COLS.length, [
//   LEFT_COLS.length,
//   MAIN_SINGLE_COLS.length,
// ]);
...
//       ...(countryName === "us" || countryName === "global"
//         ? [{ [columnsToDisplay2[0]]: "Shipment Charges (-)", [columnsToDisplay2[10]]: Math.abs(Number(totals.shipment_charges)) }]
//         : []),
...
//   // ✅ fallback to collapsed grid when GroupedCollapsibleTable hasn't reported yet
//   const fallbackCollapsedCount = LEFT_COLS.length + MAIN_SINGLE_COLS.length;
//   const effectiveColCount = Math.max(mainColCount || 0, fallbackCollapsedCount);

//   // ✅ consistent colSpan helpers
//   const labelSpan = Math.max(effectiveColCount - 2, 1);
...
//                   profit: "profit_breakdown",
//                 }}
//                 // ✅ critical: we use this count to align summary table columns
//                 onVisibleColCountChange={setMainColCount}
//                 showSignRowInBody
//                 getSignForCol={getSignForCol}
//                 getRowClassName={(_, index) => {
...
//              {SUMMARY_COL_COUNT > 0 && (
//               <table
//                 className="w-full table-fixed border-collapse text-[#414042]"
//                 style={{ tableLayout: "fixed" }}
//               >
//                 <colgroup>
//                   {Array.from({ length: effectiveColCount }).map((_, i) => (
//                     <col key={i} />
//                   ))}
//                 </colgroup>
...
//                     {/* 2nd last column (expanded value col) */}
//                     <td className="border border-gray-300 px-2 py-2" />

//                     {/* last column (end total col) */}
//                     <td className="whitespace-nowrap border border-gray-300 px-2 py-2 text-center text-[clamp(12px,0.729vw,16px)]">
//                       {formatValue(totals.advertising_total, "advertising_total")}
//                     </td>
//                   </tr>
...
//                   {/* ===================== Non-collapsible rows: value must be in END column ===================== */}
            //       {(countryName === "us" || countryName === "global") && (
            //         <tr>
            //           <td
            //             colSpan={labelSpan}
            ...
            //         {
            //           label: "Net Reimbursement",
            //           value: formatValue(Math.abs(totals.reimbursement_lost_inventory_amount), "reimbursement_lost_inventory_amount"),
            //         },
            //         { label: "TACoS (Total Advertising Cost of Sale)", value: `${formatValue(totals.acos, "acos")}%` },
            //         {
            //           label: "Reimbursement vs CM2 Margins",
            //           value: `${formatValue(totals.rembursment_vs_cm2_margins, "rembursment_vs_cm2_margins")}%`,
            //         },

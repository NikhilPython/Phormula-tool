...
const LiveLineChart: React.FC<{
  ...
  // ✅ responsive sizing (hooks MUST be above any return)
  const [isCompactView, setIsCompactView] = useState(false);
  useEffect(() => {
    const compute = () => setIsCompactView(typeof window !== "undefined" && window.innerWidth < 1536);
    compute();
  ...
  // ✅ build arrays aligned to allDays (no buildSeries fn => no deps warning)
    ...
    () => ({
      ...
      tooltip: {
        ...
        formatter: (rawParams: unknown) => {
          ...
          const lines = params
            .filter((p) => !isPadX(p?.axisValue)) // ✅ hide pad in tooltip
            .map((p) => {
              const val = p.data;
              const seriesName = stripRange(p.seriesName ?? "");
  ...
  // ✅ ResizeObserver (hook must be above return)
  useEffect(() => {
    const el = containerRef.current;
    if (!el) return;

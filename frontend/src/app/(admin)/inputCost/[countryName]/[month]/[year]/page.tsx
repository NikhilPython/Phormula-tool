import type { Metadata } from "next";
import InputCostClient from "./InputCostClient";

type Params = {
  countryName: string;
  month: string;
  year: string;
};

const monthName = (m: string) => {
  const v = String(m).toLowerCase();
  const map: Record<string, string> = {
    "01": "January", "1": "January", "jan": "January",
    "02": "February", "2": "February", "feb": "February",
    "03": "March", "3": "March", "mar": "March",
    "04": "April", "4": "April", "apr": "April",
    "05": "May", "5": "May",
    "06": "June", "6": "June", "jun": "June",
    "07": "July", "7": "July", "jul": "July",
    "08": "August", "8": "August", "aug": "August",
    "09": "September", "9": "September", "sep": "September",
    "10": "October", "oct": "October",
    "11": "November", "nov": "November",
    "12": "December", "dec": "December",
  };
  return map[v] ?? m;
};

const formatCountry = (c: string) => {
  const v = (c || "").toLowerCase();
  if (v === "uk") return "UK";
  if (v === "us") return "US";
  if (v === "india") return "India";
  if (v === "ca" || v === "canada") return "Canada";
  if (v === "eu" || v === "europe") return "Europe";
  if (v === "global") return "Global";
  return v.toUpperCase();
};

export async function generateMetadata(
  { params }: { params: Promise<Params> }
): Promise<Metadata> {
  const { countryName, month, year } = await params;

  const country = formatCountry(countryName);
  const monthFormatted = monthName(decodeURIComponent(month || ""));

  const pageTitle = `Input Cost | Amazon ${country}`;

  return {
    title: `${pageTitle} | Phormula`,
    description: `Manage SKU-level landing costs, update pricing, and analyze gross margins for Amazon ${country} in ${monthFormatted} ${year}. Upload warehouse data and track inventory insights in real time.`,
    keywords: [
      "input cost",
      "landing cost",
      "sku pricing",
      "amazon sku management",
      "gross margin analysis",
      "inventory tracking",
      "warehouse upload",
      "cost management",
      "phormula",
      `amazon ${country}`,
      `${monthFormatted} ${year} input cost`,
    ],
    robots: { index: false, follow: false },
    openGraph: {
      title: `${pageTitle} | Phormula`,
      description: `Edit SKU pricing, calculate gross margins, and manage warehouse inventory data for Amazon ${country}.`,
      type: "website",
    },
  };
}
export default function Page(
  { params }: { params: Promise<Params> }
) {
  return <InputCostClient params={params} />;
}